...
{
    MessageBoxA(NULL, szMessage, "D2Save64kb", MB_OK | MB_ICONERROR);
...
int __stdcall DllAttach()
{
	...
	BOOL bPatched = FALSE;
	DWORD version = NULL;
	LPBYTE pBase = (LPBYTE) DLLBASE_D2GAME;
	IMAGE_NT_HEADERS* pHeader;

	// Check version and patch D2Game.dll
	if (pBase != NULL) {
		pHeader = GetHeader(pBase);
		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0xC66AC) {
			version = 0x109B;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game109b);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0xC6D5C) {
			version = 0x109D;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game109d);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0xEDC2C) {
			version = 0x110F;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game110f);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0x373D) {
			version = 0x111B;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game111b);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0x374B) {
			version = 0x112A;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game112a);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0x373C) {
			version = 0x113C;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game113c);
			bPatched = TRUE;
		}

		if (pHeader->OptionalHeader.AddressOfEntryPoint == 0x3747) {
			version = 0x113D;
			D2TEMPLATE_ApplyPatch(hGame, gptD2Game113d);
			bPatched = TRUE;
		}
	}

	// Check if 'switch version' file exist
	if ((GetFileAttributes("1.09b.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game109b);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.09d.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE))  {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game109d);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.10f.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game110f);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.11b.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game111b);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.12a.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game112a);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.13c.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game113c);
		bPatched = TRUE;
	}

	if ((GetFileAttributes("1.13d.txt") != INVALID_FILE_ATTRIBUTES) && (bPatched == FALSE)) {
		D2TEMPLATE_ApplyPatch(hGame, gptD2Game113d);
		bPatched = TRUE;
	}

	//Check Plugy.dll version and patch
	DWORD crc = 0;
	crc = D2TEMPLATE_GetCRC("Plugy.dll");

	if (bPatched == TRUE) {
		if (crc == 0x304AAEC1) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy700);
		}

		if (crc == 0x1096E47C) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy800);
		}

		if (crc == 0xC3B99094) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy900);
		}

		if (crc == 0x5D180081) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1000);
		}

		if (crc == 0x128C473F) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1001);
		}

		if (crc == 0x779384FD) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1100);
		}

		if (crc == 0x9A4AFA2F) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1101);
		}

		if (crc == 0x5176DF7C) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1102);
		}
		
		if (crc == 0xCAA957F9) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1102ugf);
		}

		if (crc == 0xE43D3716) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1200);
		}

		if (crc == 0x21AB28B6) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1400);
		}

		if (crc == 0xEBDF0956) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1401);
		}

		if (crc == 0xB533F33E) {
			D2TEMPLATE_ApplyPatch(hGame, gptPlugy1402);
		}
	}
	return 1;
}
...
UINT32 __fastcall crc32(UINT32 crc, const void* buf, size_t size)
{
	const UINT8* p;
...
DWORD __fastcall D2TEMPLATE_GetCRC(char* FileName)
{
	DWORD dwBytesRead = 0;
	DWORD crc = 0;
	...
	if (hFile != INVALID_HANDLE_VALUE) {
		...
		LPVOID pBuf = VirtualAlloc(0, dwFileSize, MEM_COMMIT + MEM_RESERVE, PAGE_READWRITE);
		if (ReadFile(hFile, pBuf, dwFileSize, &dwBytesRead, NULL)) {
			crc = crc32(0, pBuf, dwFileSize);
		}
		...
		// D2TEMPLATE_FatalError("PlugY not found");
	}
